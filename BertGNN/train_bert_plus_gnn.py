...
from model import BertGNN, BertGAT

parser = argparse.ArgumentParser()
parser.add_argument('--max_length', type=int, default=128, help='Bert input length')
parser.add_argument('--batch_size', type=int, default=64)
parser.add_argument('-m', '--m', type=float, default=0.7, help='BERT and gnn prediction factor balancing ')
parser.add_argument('--nb_epochs', type=int, default=50)
parser.add_argument('--bert_init', type=str, default='roberta-base',
                    choices=['roberta-base', 'roberta-large', 'bert-base-uncased', 'bert-large-uncased'])
parser.add_argument('--pretrained_bert_ckpt', default=None)
parser.add_argument('--dataset', default='R8', choices=['R8', 'R52', 'ohsumed'])
parser.add_argument('--checkpoint_dir', default=None, help='checkpoint directory, [bert_init]_[gnn_model]_[dataset] if not specified')
parser.add_argument('--gnn_model', type=str, default='gnn', choices=['gnn', 'gat'])
parser.add_argument('--gnn_layers', type=int, default=2)
parser.add_argument('--n_hidden', type=int, default=200, help='the dimension of gnn hidden layer, the dimension for gat is n_hidden * heads')
...
if checkpoint_dir is None:
    ckpt_dir = './checkpoint/{}_{}_{}'.format(bert_init, gnn_model, dataset)
else:
...
cpu = th.device('cpu')
gpu = th.device('cuda:{}'.format(th.cuda.current_device()) 
                       if th.cuda.is_available() else 'cpu')

logger.info('arguments:')
logger.info(str(args))
logger.info('checkpoints are saved in {}'.format(ckpt_dir))
# Model
...
# instantiate model based on class number
if gnn_model == 'gnn':
    model = BertGNN(nb_class=nb_class, pretrained_model=bert_init, m=m, gnn_layers=gnn_layers,
                    n_hidden=n_hidden, dropout=dropout)
else:
    model = BertGAT(nb_class=nb_class, pretrained_model=bert_init, m=m, gnn_layers=gnn_layers,
...
# transform one-hot label to class ID 
